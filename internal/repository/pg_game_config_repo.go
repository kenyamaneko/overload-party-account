...
// PgGameConfigRepository は PostgreSQL を使用した GameConfigRepo の実装です。
type PgGameConfigRepository struct {
	pool *pgxpool.Pool
}

// NewPgGameConfigRepository は PgGameConfigRepository を生成します。
func NewPgGameConfigRepository(pool *pgxpool.Pool) *PgGameConfigRepository {
	return &PgGameConfigRepository{pool: pool}
}

// GetInt64 は指定キーの設定値を int64 で返します。キーが存在しなければ fallback を返します。
