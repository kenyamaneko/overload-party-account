...
| ファクション所有 | onboarding 完了時の初期 faction 登録（`player-onboarded` イベント）と shop 由来の追加（`faction-acquired` イベント）を `player_factions` に射影 |
...
ADR-022 により、かつて `faction-selected` topic が担っていた 2 業務事実（scenario 初期選択 / shop 購入）は業務事実単位で分解された。scenario 初期選択は §9.3 `player-onboarded` に統合され、shop 起因の faction 所有権成立は本 topic で扱う。ADR-031 で shop 内の `faction-purchased` がさらに card pack 配布側 (`card-pack-purchased`) と faction 所有権側 (`faction-acquired`) に分割され、後者を account / gateway が購読する。
